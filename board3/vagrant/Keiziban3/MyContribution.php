<?php
    ...
    if(isset($_POST['Logoutbutton'])){  //ログアウトボタンが押されたときの処理
        ...
        header('Location: http://192.168.33.10/vagrant/Keiziban3/Login.php');  //ログイン画面に戻る
    }

    if(isset($_POST['homebutton'])){   //ホームボタンが押されたときの処理
        header('Location: http://192.168.33.10/vagrant/Keiziban3/Keiziban3.php');   //ユーザidの情報をセッションで保持したままホーム画面へ移動
    }
    ...
    for ($i = 1; $i < count($fetchAll[0]); $i++){
        if(isset($_POST["deletebutton{$i}"])){   //どのボタンが押されたか
            ...
            header('Location: http://192.168.33.10/vagrant/Keiziban3/MyContribution.php');
        }
    }

...
class LC_Page_Admin_Contents extends LC_Page_Admin_Ex
{
    ...
    public function lfCheckError(&$objFormParam)
    {
        ...
        $objErr->doFunc(array('表示終了日', 'finish_year', 'finish_month', 'finish_day'), array('CHECK_DATE'));
        $objErr->doFunc(array('表示開始日', '表示終了日', 'start_year', 'start_month', 'start_day', 'finish_year', 'finish_month', 'finish_day'), array('CHECK_SET_TERM'));
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        /*新たに記述*/
        $objFormParam->addParam('表示開始日(年)', 'start_year', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('表示開始日(月)', 'start_month', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('表示開始日(日)', 'start_day', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('表示終了日(年)', 'finish_year', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('表示終了日(月)', 'finish_month', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('表示終了日(日)', 'finish_day', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
    }
    ...
    public function doRegist($news_id, $sqlval, SC_Helper_News_Ex $objNews)
    {
        ...
        /*新たに記述*/
        $sqlval['start_date'] = $this->getRegistStartDate($sqlval);
        $sqlval['finish_date'] = $this->getRegistFinishDate($sqlval);

        unset($sqlval['year'], $sqlval['month'], $sqlval['day']);

        /*新たに記述*/
        unset($sqlval['start_year'], $sqlval['start_month'], $sqlval['start_day']);
        unset($sqlval['finish_year'], $sqlval['finish_month'], $sqlval['finish_day']);
     ...
     * @return string 登録日を示す文字列
     */
    public function getRegistStartDate($arrPost)
    {
        if ($arrPost['start_year'] != '' && $arrPost['start_month'] != '' && $arrPost['start_day'] != '') {
            $registStartDate = $arrPost['start_year'] .'/'. $arrPost['start_month'] .'/'. $arrPost['start_day'];
     }
    else $registStartDate = NULL;
    ...
    public function getRegistFinishDate($arrPost)
    {
      if ($arrPost['finish_year'] != '' && $arrPost['finish_month'] != '' && $arrPost['finish_day'] != '')
      {
            $registFinishDate = $arrPost['finish_year'] . '/' . $arrPost['finish_month'] . '/' . $arrPost['finish_day'];
            $end_date = date('Y/m/d', strtotime('1 day', strtotime($registFinishDate)));
      }
      else $end_date = NULL;

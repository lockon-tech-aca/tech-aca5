...
class SC_Helper_News
{
    ...
    public static function getNews($news_id, $has_deleted = false)
    {
        ...
        $col = '*, cast(news_date as date) as cast_news_date, cast(start_time as date) as cast_start_time, cast(end_time as date) as cast_end_time';
        $where = 'news_id = ?';
        if (!$has_deleted) {
            $where .= ' AND del_flg = 0';
        }
    ...
    public function getList($dispNumber = 0, $pageNumber = 0, $has_deleted = false)
    {
        ...
        // 公開期限チェック
        $arrRet = $this->displayNews($arrRet);
     ...
     * ニュースの公開期限チェック.
     *
     * @param  array $arrRet ニュースのリスト
     * @return array $arrRet 公開フラグ編集後のリスト
     */
    private function displayNews($arrRet)
    {
        $now = date("Y/m/d");

        foreach ($arrRet as $key => $val) {
            // 表示フラグ：0が表示、1が非表示
            $start = date('Y/m/d', strtotime($val['start_time']));
            $end   = date('Y/m/d', strtotime($val['end_time']));
            ...
            $arrRet[$key]['hidden_flg'] = $hidden_flg;
        }

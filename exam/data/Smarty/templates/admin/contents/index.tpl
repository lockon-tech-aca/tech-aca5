    ...
    <form name="move" id="move" method="post" action="?">
        ...
        <table class="list">
            ...
            <tr style="background:<!--{if $arrNews[data].news_id != $tpl_news_id}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}-->;" class="center">
                ...
                <td><!--{if $arrNews[data].hidden_flg == 0}--><p>公開中</p>
                　　<!--{else}--><p>非公開</p><!--{/if}--></td>
                <td><!--{if $arrNews[data].cast_start_time}--><!--{$arrNews[data].cast_start_time|date_format:"%Y/%m/%d"}--><!--{else}--><p>指定なし</p><!--{/if}--></td>
                <td><!--{if $arrNews[data].cast_end_time}--><!--{$arrNews[data].cast_end_time|date_format:"%Y/%m/%d"}--><!--{else}--><p>指定なし</p><!--{/if}--></td>
                <td class="left">

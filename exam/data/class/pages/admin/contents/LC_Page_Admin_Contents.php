...
class LC_Page_Admin_Contents extends LC_Page_Admin_Ex
{
    ...
    public function action()
    {
        ...
        switch ($this->getMode()) {
            ...
            case 'pre_edit':
                ...
                list($news['start_year'],$news['start_month'],$news['start_day']) = $this->splitNewsDate($news['cast_start_time']);
                list($news['end_year'],$news['end_month'],$news['end_day'])       = $this->splitNewsDate($news['cast_end_time']);
    ...
    public function lfCheckError(&$objFormParam)
    {
        ...
        $objErr->doFunc(array('公開開始日', '最終公開日', 'start_year', 'start_month', 'start_day', 'end_year', 'end_month', 'end_day'), array('CHECK_SET_TERM_FOR_CONTENTS'));
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        $objFormParam->addParam('公開開始(年)', 'start_year', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('公開開始(月)', 'start_month', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('公開開始(日)', 'start_day', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終公開(年)', 'end_year', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終公開(月)', 'end_month', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終公開(日)', 'end_day', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
    ...
    public function doRegist($news_id, $sqlval, SC_Helper_News_Ex $objNews)
    {
        ...
        $date_array = $this->getRegistDate($sqlval);
        $sqlval['news_date']   = $date_array['news_date'];
        $sqlval['start_time']  = $date_array['start_time'];
        $sqlval['end_time']    = $date_array['end_time'];
    ...
    {
        $Date['news_date']  = $arrPost['year'] .'/'. $arrPost['month'] .'/'. $arrPost['day'];

        if (isset($arrPost['start_year']) && $arrPost['start_month'] && $arrPost['start_day']) {
          $Date['start_time'] = $arrPost['start_year'] .'/'. $arrPost['start_month'] .'/'. $arrPost['start_day'];
        }
        if (isset($arrPost['end_year']) && $arrPost['end_month'] && $arrPost['end_day']) {
          $Date['end_time']   = $arrPost['end_year'] .'/'. $arrPost['end_month'] .'/'. $arrPost['end_day'];
        }

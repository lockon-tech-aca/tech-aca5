...
class SC_Helper_News
{
    ...
    public static function getNews($news_id, $has_deleted = false)
    {
        ...
        /*新たに記述*/
        $col = '*, cast(news_date as date) as cast_news_date, cast(start_date as date) as cast_start_date, cast(finish_date as date) as cast_finish_date';
        $where = 'news_id = ?';
        if (!$has_deleted) {
            $where .= ' AND del_flg = 0';
        }
    ...
    /*新たに記述*/
    public function getlimitList($dispNumber = 0, $pageNumber = 0, $has_deleted = false)
    {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $col = '*, cast(news_date as date) as cast_news_date';
        /* $where = ""; */
        $where = '(start_date IS NULL OR start_date <= CURRENT_TIMESTAMP)';
        $where .= ' AND (finish_date IS NULL OR CURRENT_TIMESTAMP < finish_date)';
        if (!$has_deleted) {
            $where .= ' AND del_flg = 0';
        }
        $table = 'dtb_news';
